...
def _map_roles_to_ids(players: List[Dict]) -> Dict[str, str]:
    ...
    This allows the playbook to use abstract roles ('team1-pg') while we move real IDs.
    """
    role_map = {}
    ...
    for p in offense_players:
        # We stored 'PG', 'SG', etc. in the 'position' data field during setup
        pos = p['data'].get('position') 
        if pos:
            role_map[pos] = p['id']
...
def executor_node(state: AgentState) -> Dict[str, Any]:
    ...
    # ---------------------------------------------------------
    # Case A: Execute a Pre-defined Play (Playbook)
    # ---------------------------------------------------------
    if intent == "PLAYBOOK":
        play = state.get("selected_play")
        
        if play and "steps" in play and len(play["steps"]) > 0:
            # We execute the LAST step to show the final result of the play immediately
            final_step_data = play["steps"][-1] 
            
            # Create map: 'PG' -> 'lal-lebron-james'
            role_to_real_id = _map_roles_to_ids(current_players)
            
            # Track where the ball ends up in this step
            ball_pos_in_step = None

            # 1. Process positions from the Playbook
            for item in final_step_data:
                generic_id = item.get("id") # e.g. "team1-pg" or "ball"
                ...
                # Convert "team1-pg" -> "PG"
                if generic_id.startswith("team1-"):
                    role = generic_id.split("-")[1].upper() # Extract "PG", "SG", etc.
                    
                    # Find the real player ID for this role
                    real_id = role_to_real_id.get(role)
                    if real_id:
                        updates_map[real_id] = target_pos

            # 2. Determine new Ball Handler
            # If a player ends up at the exact same location as the ball, they have it.
            if ball_pos_in_step:
                # Reset handler initially, then check if anyone caught it
                new_ball_handler = None 
                ...
                # Fallback: If no one moved to ball, check if static players have it
                if not new_ball_handler:
                    for p in current_players:
                        # Check players who didn't move
                        if p['id'] not in updates_map: 
                             if (p['position']['x'] == ball_pos_in_step['x'] and 
                                 p['position']['y'] == ball_pos_in_step['y']):
                                 new_ball_handler = p['id']
                                 break

            action_description = f"Executed {play['name']}"
    ...
    for player in current_players:
        ...
        # If this player is the Ball Handler, the ball snaps to them
        if p_id == new_ball_handler:
            current_ball_pos = new_props["position"]
            
        updated_players_list.append(new_props)

    return {
        "players": updated_players_list,
        "ball_position": current_ball_pos,
